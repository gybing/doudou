...
public class MediaplayerActivity extends LinearLayout{
	...
	private MusiclistActivity currentListItem = null;
	...
	public MediaplayerActivity(Context context, AttributeSet attrs) {
				...
				if (currentState == Player_State.INIT) {
					if (null != currentListItem) {
						...
						if (!musicList.isEmpty()) {
							play((MusiclistActivity)mlistP.getChildAt(0));
						}
					}
		...
		mlistP = (LinearLayout)findViewById(R.id.mlistP);
		
		musicList = new ArrayList<Material>();
	}
	
	public void test() {
		if (mlistP.getChildCount() == 5) {
			mlistP.removeAllViews();
		}
		Button b = new Button(context);
		b.setText("asdfasdfasdf");
		mlistP.addView(b);
	}
	...
	public void insertMlist(List<Material> materialList) {  //插入多媒体列表
		...
		for (Material musicPath : musicList) {
			MusiclistActivity listItem = new MusiclistActivity(context , null);
			...
			listItem.setOnClickListener(new OnClickListener() {
				...
				@Override
				public void onClick(View arg0) {
					if (currentListItem == (MusiclistActivity)arg0) {
						...
						stop();
						play((MusiclistActivity)arg0);
					}
				}
			});
			
			mlistP.addView(listItem);
	...
	private void play(MusiclistActivity v){ 
		try{     
			...
			mMediaPlayer.reset();     
			System.out.println(Constants.SD_PATH + v.getMusicPath());
	...
	private void playPre() {
		if (null == currentListItem) {
			...
			currentListItem.editLength("");
			stop();
			play((MusiclistActivity)mlistP.getChildAt(currentListItem.index-1));
		}
	}
	...
	private void playNext() {
		if (null == currentListItem) {
			return;
		} else if (currentListItem.index == mlistP.getChildCount()-1) {
