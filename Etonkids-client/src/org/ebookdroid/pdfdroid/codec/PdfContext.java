...
import com.doudoumobile.etonkids_client.LessonActivity;
...
public class PdfContext extends AbstractCodecContext {
    ...
    public static int getWidthInPixels(final float pdfWidth) {
        return getSizeInPixels(pdfWidth, LessonActivity.DM.xdpi);
    }

    public static int getHeightInPixels(final float pdfHeight) {
        return getSizeInPixels(pdfHeight, LessonActivity.DM.ydpi);
    }
    ...
    private static int getDensityDPI() {
        try {
            if (DENSITY_DPI_FIELD != null) {
                return ((Integer) DENSITY_DPI_FIELD.get(LessonActivity.DM)).intValue();
            }
        } catch (final Throwable ex) {
            ex.printStackTrace();
        }
