			...
			while (it.hasNext()) {
				...
				if (item.isFormField()) {
					...
					if (item.getName() != null && !item.getName().equals("")) {
						...
						// 用于获取file中的文件名（不包含路径）
						String picName = "Head_" + DateUtil.getInstance().getDetailStringFromDate(now) + poster;
						...
						MayayaBackend.getInstance().addHeadPicToQueue(file.toString());
						childService.updateHeadPic(childId, savePath+"/"+tempFile.getName());
					} else {
